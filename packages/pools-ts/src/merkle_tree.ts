...
export class MerkleTree {
  ...
  constructor({
    ...
    for (let i = 0; i < this.LEVELS; i++) {
      this._zeroValues[i + 1] = poseidon(
        [this._zeroValues[i], this._zeroValues[i]],
        1
      );
    }
    if (!Array.isArray(leaves)) {
      ...
      throw new Error(
        `Leaves length exceeds the maximum capacity of the tree.`
      );
    }
  ...
  public insert(leaf: BigNumberish): number {
    ...
    if (index >= this.CAPACITY) {
      throw new Error(
        `The tree is at capacity. No more leaves can be inserted.`
      );
    }
    try {
  ...
  public update(index: number, leaf: BigNumberish) {
    ...
    if (index >= this.length) {
      throw new Error(
        `Index ${index} is out of bounds of tree with size ${this.length}.`
      );
    }
    try {
      leaf = BigNumber.from(leaf);
  ...
  protected _update(index: number, leaf: BigNumber) {
    ...
    for (let i = 1; i < this.LEVELS + 1; i++) {
      ...
      const p = n >>> 1;
      if ((n & 1) === 1) {
        this._layers[i][p] = poseidon(
          [this._layers[i - 1][n - 1], this._layers[i - 1][n]],
          1
        );
      } else {
        if (n + 1 >= this._layers[i - 1].length) {
          this._layers[i][p] = poseidon(
            [this._layers[i - 1][n], this._zeroValues[i - 1]],
            1
          );
        } else {
          this._layers[i][p] = poseidon(
            [this._layers[i - 1][n], this._layers[i - 1][n + 1]],
            1
          );
        }
      }
  ...
  static fullEmpty({
    ...
    let layers: BigNumber[][] = new Array(tree.LEVELS + 1);
    for (let i = 0; i <= tree.LEVELS; i++) {
      layers[i] = new Array(Math.ceil(treeLength / (1 << i))).fill(
        tree.zeroValues[i]
      );
    }
    tree._layers = layers;
