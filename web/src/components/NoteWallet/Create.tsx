...
import { NoteWalletV2 } from 'note-wallet';
...
export default function Create() {
  ...
  const [tempEncryptedMnemonic, setTempEncryptedMnemonic] = useState<EncryptedJson>();
  ...
  const handleTabChange = (index: number) => {
    if (index === 0) {
      ...
      setTempEncryptedMnemonic(undefined);
    }
    setCurrentTab(index);
  };
  ...
  const handleMnemonicInput = (e: any) => {
    setMnemonicError('')
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = (event) => {
    ...
    try {
      const result = zxcvbn(_password.toString());
      ...
      if (score < MIN_PW_STRENGTH) {
        ...
        setEncryptProgress;
        _noteWallet
          .encryptToJson(_password.toString())
      ...
      <Flex width="100%" justifyContent="center">
        ...
        >
          <TabList mb="1em" height="50px">
            ...
            <Tab _selected={{ color: 'white', bg: 'gray.900' }} isDisabled={!tempWallet || currentTab === 2}>
              <Text fontSize="sm">2 Create Password</Text>
            </Tab>
            <Tab _selected={{ color: 'white', bg: 'gray.900' }} isDisabled={!tempEncryptedMnemonic}>
                ...
                <form>
                  <VStack spacing={4} align="stretch">
                    <FormControl isInvalid={!!mnemonicError}>
                      ...
                      <Textarea
                        value={mnemonic || ''}
                        onChange={(e) => handleMnemonicInput(e)}
                    ...
                    <Button
                      onClick={(e) => {
                        e.preventDefault();
                        setMnemonic(ethers.Wallet.createRandom({
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (mnemonic.trim().split(' ').length !== 12) {
                          ...
                          try {
                            setTempWallet(ethers.Wallet.fromMnemonic(mnemonic));
                            setMnemonicError('');
                            setCurrentTab(1);
                          } catch {
